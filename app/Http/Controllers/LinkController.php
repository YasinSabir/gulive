...
class LinkController extends BaseController
{
     ...
     * @return Response
     */
    public function store_api(CrupdateLinkRequest $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json([
                ...
                'Message' => 'long_url field is missing!'], Response::HTTP_BAD_REQUEST);
        }
        ...
        $arr      = explode('/',$link['short_url']);
        $emoji    = end($arr);
        $removed  = array_pop($arr);
        array_push( $arr,urldecode($emoji));
        $short_link = implode('/',$arr);

        $export_link = [
            'long_url'  => $link['long_url'],
            'short_url' => $short_link,
            'msg'       => 'Successfully Link Shorted'
        ];

        return response()->json(['data' => $export_link, 'status' => 200]);
    }
