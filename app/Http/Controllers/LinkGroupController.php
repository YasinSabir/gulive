...
class LinkGroupController extends BaseController
{
    ...
    public function show(LinkGroup $linkGroup)
    {
        ...
        $response = ['group' => $linkGroup];

        if ( ! $this->request->get('skipStats')) {
            $response['reports'] = app(GenerateLinkReport::class)->execute($this->request->all(), $linkGroup);
        }

        if ( ! $this->request->get('reportsOnly')) {
            $params = $this->request->all();
            $params['groupId'] = $linkGroup->id;
            $response['links'] = app(PaginateLinks::class)->execute($params);
        }
