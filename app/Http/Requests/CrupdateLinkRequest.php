...
class CrupdateLinkRequest extends BaseFormRequest
{
    ...
    public function rules()
    {
        ...
        $aliasMin = app(Settings::class)->get('links.alias_min', 5);
        $aliasMax = app(Settings::class)->get('links.alias_max', 10);

        $rules = [
            'alias' => "nullable|string|min:$aliasMin|max:$aliasMax|unique:links,alias,$except",
            'long_url' => "$longUrlRequired|max:250", // TODO: can't use "url" validation on 7.3 until laravel is upgraded
            'domain_id' => "nullable|integer",
            'password' => 'nullable|string|max:250',
            'disabled' => 'nullable|boolean',
            'description' => 'nullable|string|max:250',
            'expires_at' => 'nullable|multi_date_format:Y-m-d H:i:s,Y-m-d',
            'pixels.*' => 'required|int',
            'rules' => 'array',
